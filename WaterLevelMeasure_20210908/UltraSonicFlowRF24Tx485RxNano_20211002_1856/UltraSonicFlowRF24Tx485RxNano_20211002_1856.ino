...
#define RS485_TX_PIN  A0  // 18  // A0  // RS485 Driver Input     NANO=A0 / MEGA2560=18
#define RS485_DE_PIN  A6  // RS485 Drive Enable            NANO
#define RS485_RE_PIN  A7  // RS485 Rx Enable (active low)  NANO
#define RS485_RX_PIN  A1  // 19  // A1  // RS485 Rx Output        NANO=A1 / MEGA2560=19
...
//  lcd_print("Rx ");
  ...
  //NeoSWSerial.attachInterrupt( handleRxChar ); // Nano
...
void loop()
  {
#ifdef TEST_MODE_INTR
  intrCount1++;
#endif
#ifdef TEST_MODE_NEOSW
    //if (Serial1.available()>0) { // MEGA
    if (NeoSWSerial.available()>0) { // Nano
      Serial.print("\t\t\t\t485RX=");
      for (index=0; index<max_payload_size; index++) {
        //receive_payload = NeoHWSerial.read();
        //if (Serial1.available()>0) { // MEGA
        if (NeoSWSerial.available()>0) { // Nano
          //incomingByte = Serial1.read(); // MEGA
          incomingByte = NeoSWSerial.read(); // Nano
          receive_payload[index] = incomingByte;
          //NeoSWSerial.write( receive_payload );
          //Serial.println(receive_payload);
          Serial.print(incomingByte);
        }
        else {
          receive_payload[index] = 0;
          break;
        }
      }
      Serial.println("=");
